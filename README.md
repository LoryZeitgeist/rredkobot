...
1. Скопируйте `.env.example` в `.env` и заполните значения:
   ...
   - `PRESETS_DIR` — путь к папке с изображениями (по умолчанию `./presets` относительно корня проекта).
   - `STATE_FILE` — путь к файлу с назначениями пресетов (по умолчанию `./data/chat_presets.json` относительно корня проекта).

   Относительные пути автоматически резолвятся от каталога проекта, поэтому переселение на новый сервер не требует ручного переписывания путей вроде `/app/...`.
